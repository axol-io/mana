defmodule ExWire.DVT.CryptoTest do
  ...
  @test_message_binary <<72, 101, 108, 108, 111, 44, 32, 68, 86, 84, 32, 87, 111, 114, 108, 100, 33>>
  ...
  describe "threshold configuration validation" do
    ...
    test "rejects invalid threshold configurations" do
      ...
      assert {:error, :invalid_threshold} = Crypto.validate_threshold_config(1, 3) # Less than majority
    end
  ...
  describe "threshold key generation" do
    test "generates threshold keys for valid configuration" do
      ...
      assert {:ok, {public_key_set, key_shares}} = Crypto.generate_dvt_keys(threshold, total_nodes)
  ...
  describe "signature share creation" do
    ...
    test "creates signature shares", %{key_shares: key_shares} do
      ...
      assert {:ok, signature_share} = Crypto.create_dvt_signature_share(key_share, @test_message_binary)
    ...
    test "creates signature shares using HSM", %{threshold: threshold, total_nodes: total_nodes} do
      ...
      case Crypto.generate_dvt_keys(threshold, total_nodes, hsm_config) do
        {:ok, {_public_key_set, hsm_references}} ->
          ...
          assert {:ok, signature_share} = Crypto.create_dvt_signature_share(
            hsm_key_share, @test_message_binary, hsm_config
          )
          assert is_binary(signature_share)
  ...
  describe "signature aggregation" do
    ...
    test "aggregates signature shares successfully", 
         %{public_key_set: public_key_set, signature_shares: signature_shares, threshold: threshold} do
      
      assert {:ok, threshold_signature} = Crypto.aggregate_dvt_signatures(
        public_key_set, signature_shares, threshold
      )
    ...
    test "fails with insufficient shares",
      ...
      insufficient_shares = Enum.take(signature_shares, 2) # Less than threshold
      
      assert {:error, :insufficient_shares} = Crypto.aggregate_dvt_signatures(
        public_key_set, insufficient_shares, 3
      )
    end
    ...
    test "aggregation is consistent",
      ...
      assert {:ok, sig1} = Crypto.aggregate_dvt_signatures(public_key_set, signature_shares, threshold)
      assert {:ok, sig2} = Crypto.aggregate_dvt_signatures(public_key_set, signature_shares, threshold)
  ...
  describe "signature verification" do
    setup do
      ...
      {:ok, threshold_signature} = Crypto.aggregate_dvt_signatures(
        public_key_set, signature_shares, threshold
      )
    ...
    test "verifies valid threshold signatures", 
      ...
      assert {:ok, true} = Crypto.verify_dvt_signature(
        public_key, threshold_signature, @test_message_binary
      )
    end
    ...
    test "rejects invalid threshold signatures",
      ...
      invalid_signature = <<0::256>> # Invalid signature
      
      assert {:ok, false} = Crypto.verify_dvt_signature(
        public_key, invalid_signature, @test_message_binary
      )
    end
    ...
    test "rejects signatures for wrong messages",
      ...
      assert {:ok, false} = Crypto.verify_dvt_signature(
        public_key, threshold_signature, wrong_message
      )
    end
  ...
  describe "DKG operations" do
    test "initializes DKG round successfully" do
      ...
      assert length(initial_shares) == length(participants) - 1 # Excludes own node
    end
    ...
    test "generates and verifies DKG complaints" do
      ...
      assert {:ok, complaint} = Crypto.generate_complaint(
        complainer_id, accused_id, invalid_share, round_id
      )
  ...
  describe "error handling" do
    test "handles corrupted key shares gracefully" do
      corrupted_share = <<1, 2, 3>> # Too short to be valid
      
      assert {:error, _reason} = Crypto.create_dvt_signature_share(
        corrupted_share, @test_message_binary
      )
    end

    test "handles invalid public key sets" do
      invalid_pks = <<0::64>> # Too short
    ...
    test "handles network/serialization errors" do
      ...
      edge_cases = [
        "", # Empty binary
        <<0>>, # Single byte
        :crypto.strong_rand_bytes(1000) # Large random data
      ]
