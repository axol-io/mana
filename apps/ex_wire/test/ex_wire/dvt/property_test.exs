defmodule ExWire.DVT.PropertyTest do
  ...
  property "valid threshold configurations are always accepted" do
    check all total_nodes <- StreamData.integer(3..50),
              threshold <- StreamData.integer(div(total_nodes, 2) + 1..total_nodes) do
  ...
  property "invalid threshold configurations are always rejected" do
    check all total_nodes <- StreamData.integer(1..50),
              threshold <- StreamData.one_of([
                StreamData.integer(0..0), # Zero threshold
                StreamData.integer(total_nodes + 1..total_nodes + 10), # Threshold too high
                StreamData.integer(1..div(total_nodes, 2)) # Threshold too low for security
              ]) do
      
      assert {:error, :invalid_threshold} = Crypto.validate_threshold_config(threshold, total_nodes)
    end
  end

  # Key generation properties
  property "generated keys are always unique" do
    check all threshold <- StreamData.integer(2..7),
              total_nodes <- StreamData.integer(threshold..10),
              _iterations <- StreamData.integer(1..5) do
      
      # Generate multiple key sets
      key_sets = Enum.map(1..3, fn _ ->
  ...
  property "key generation produces correct number of shares" do
    check all threshold <- StreamData.integer(2..7),
              total_nodes <- StreamData.integer(threshold..10) do
  ...
  property "signature shares are deterministic for same input" do
    check all threshold <- StreamData.integer(2..5),
              total_nodes <- StreamData.integer(threshold..7),
              message <- StreamData.binary(min_length: 1, max_length: 1000) do
  ...
  property "signature aggregation is consistent" do
    check all threshold <- StreamData.integer(2..4),
              total_nodes <- StreamData.integer(threshold..6),
              message <- StreamData.binary(min_length: 1, max_length: 100) do
  ...
  property "insufficient shares always fail aggregation" do
    check all threshold <- StreamData.integer(3..7),
              total_nodes <- StreamData.integer(threshold..10),
          ...
          case insufficient_shares_result do
            {:ok, insufficient_shares} ->
              ...
              assert {:error, :insufficient_shares} = 
                Crypto.aggregate_dvt_signatures(public_key_set, insufficient_shares, threshold)
  ...
  property "DKG initialization produces valid data structures" do
    check all participants <- StreamData.list_of(
                StreamData.integer(0..20), 
                min_length: 3, 
                max_length: 10
              ) |> StreamData.map(&Enum.uniq/1),
              threshold <- StreamData.integer(div(length(participants), 2) + 1..length(participants)),
              node_id <- StreamData.member_of(participants),
              round_suffix <- StreamData.binary(min_length: 1, max_length: 20) do
      
      participants = Enum.sort(participants) # Ensure consistent ordering
  ...
  property "corrupted inputs always return errors gracefully" do
    check all corrupted_data <- StreamData.one_of([
                StreamData.binary(min_length: 0, max_length: 10), # Too short
                StreamData.binary(min_length: 10000, max_length: 20000), # Very large
                StreamData.constant(<<255, 255, 255, 255>>) # Invalid data
              ]),
              message <- StreamData.binary(min_length: 1, max_length: 100) do
  ...
  property "key generation time scales reasonably with cluster size" do
    check all cluster_sizes <- StreamData.list_of(
      ...
      timing_results = Enum.map(cluster_sizes, fn total_nodes ->
  ...
  defp generate_complete_signature(threshold, total_nodes, message) do
    with {:ok, {public_key_set, key_shares}} <- Crypto.generate_dvt_keys(threshold, total_nodes),
         ...
         {:ok, threshold_signature} <- Crypto.aggregate_dvt_signatures(public_key_set, signature_shares, threshold),
