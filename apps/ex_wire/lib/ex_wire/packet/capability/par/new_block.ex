defmodule ExWire.Packet.Capability.Par.NewBlock do
  @moduledoc """
  ...
  alias ExWire.Packet.Capability.Par.Transactions
  ...
  ## Examples
      ...
      iex> %ExWire.Packet.Capability.Par.NewBlock{}
      ...> |> ExWire.Packet.Capability.Par.NewBlock.handle()
      :ok
  """
  @impl true
  ...
  def handle(_packet = %__MODULE__{}) do
    ...
    Logger.info(fn ->
      "[Par.NewBlock] Received block ##{packet.header.number} with difficulty #{packet.total_difficulty}"
    end)
