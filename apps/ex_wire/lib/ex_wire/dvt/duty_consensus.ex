defmodule ExWire.DVT.DutyConsensus do
  ...
  # Server state
  defstruct [
    :cluster_configs,     # %{cluster_id => cluster_config}
    :active_consensus,    # %{duty_key => consensus_state} 
    :duty_queue,         # Priority queue of upcoming duties
    :slashing_db,        # Anti-slashing database
    :performance_stats,  # Performance monitoring
    :audit_config,       # Audit configuration
    :beacon_config       # Beacon chain configuration
  ]
  ...
  @type consensus_state :: %{
    ...
    view: pos_integer(),              # Current view number for view-change protocol
    phase: :prepare | :commit | :decided,            # :prepare | :commit | :decided
    prepare_votes: map(),    # %{sender_id => consensus_message}
    commit_votes: map(),     # %{sender_id => consensus_message}  
    decision: term(),         # Final consensus decision
    participants: list(pos_integer()),     # List of participant node IDs
    threshold: pos_integer(),        # BFT threshold (f + 1 where f = max faulty nodes)
    leader_id: pos_integer(),        # Current view leader
    timeout_ref: reference(),      # Timer reference for timeouts
    started_at: pos_integer(),       # Consensus start time
    audit_config: map()      # Audit logging configuration
  }
  ...
  def handle_call({:register_cluster, cluster_id, cluster_config}, _from, state) do
    ...
    case validate_cluster_config(cluster_config) do
      :ok ->
        ...
        audit_duty_event(:cluster_registered, cluster_id, %{
      ...
      cluster_config ->
        case validate_duty_assignment(duty_assignment, cluster_config, state) do
          :ok ->
            ...
            consensus_state = create_consensus_state(
              cluster_id, 
              duty_assignment, 
              cluster_config, 
              state.audit_config
            ...
            audit_duty_event(:duty_submitted, cluster_id, %{
          ...
          {:error, reason} = error ->
            audit_duty_event(:duty_rejected, cluster_id, %{
              reason: reason,
              duty_type: duty_assignment.duty_type,
              slot: duty_assignment.slot
            }, state.audit_config)
        ...
        case process_message_for_consensus(message, consensus_state) do
          ...
          {:error, reason} ->
            Logger.warning("Failed to process consensus message", 
              duty_key: duty_key, reason: reason, message: message)
  ...
  def handle_cast({:trigger_view_change, duty_key}, state) do
    case Map.get(state.active_consensus, duty_key) do
      ...
      consensus_state ->
        ...
        audit_duty_event(:view_change_triggered, consensus_state.cluster_id, %{
          duty_key: duty_key,
  ...
  def handle_info(:periodic_cleanup, state) do
    ...
    active_consensus = Enum.reduce(state.active_consensus, %{}, fn {duty_key, consensus}, acc ->
      if DateTime.diff(current_time, consensus.started_at, :second) < 300 do # 5 minute limit
        Map.put(acc, duty_key, consensus)
      else
        # Audit expired consensus
        audit_duty_event(:consensus_expired, consensus.cluster_id, %{
          duty_key: duty_key,
  ...
  defp validate_duty_assignment(duty, cluster_config, state) do
  ...
  defp validate_duty_timing(duty) do
    ...
    cond do
      ...
      DateTime.diff(duty.deadline, current_time, :second) < 4 -> # Less than 4 seconds
  ...
  defp check_duty_specific_slashing(duty, previous_duties) do
    case duty.duty_type do
      :block_proposal ->
        ...
        same_slot_proposals = Enum.filter(previous_duties, fn prev ->
      ...
      _ ->
        ...
        duplicates = Enum.filter(previous_duties, fn prev ->
  ...
  defp check_attestation_slashing(duty, previous_attestations) do
    ...
    same_slot_attestations = Enum.filter(previous_attestations, fn att ->
  ...
  defp start_consensus_process(duty_key, consensus_state) do
    ...
    timeout_ms = 12_000 # 12 seconds for ETH2 slot time
  ...
  defp initiate_prepare_phase(duty_key, consensus_state) do
    ...
    prepare_message = create_consensus_message(
      :prepare,
  ...
  defp process_prepare_message(message, consensus_state) do
    ...
    if map_size(prepare_votes) >= consensus_state.threshold do
      ...
      if consensus_state.leader_id == get_node_id() do
        commit_message = create_consensus_message(
          :commit,
          consensus_state.duty_assignment,
          consensus_state.view,
          get_node_id()
  ...
  defp process_commit_message(message, consensus_state) do
    ...
    if map_size(commit_votes) >= consensus_state.threshold do
      ...
      final_consensus = %{updated_consensus | 
        phase: :decided, 
        decision: decision
      }
  ...
  defp process_view_change_message(message, consensus_state) do
    ...
    if new_view > consensus_state.view do
      ...
      updated_consensus = %{consensus_state |
        view: new_view,
  ...
  defp check_consensus_completion(consensus_state) do
    case consensus_state.phase do
      ...
      _ ->
        ...
        if duration > 15 do # 15 second hard limit
          {:failed, :timeout}
        else
          :in_progress
        end
  ...
  defp extract_consensus_decision(consensus_state) do
    ...
    payload_counts = Enum.reduce(commit_payloads, %{}, fn payload, acc ->
  ...
  defp execute_consensus_decision(duty_key, decision, state) do
    ...
    case decision.duty_type do
      ...
      :sync_committee ->
        execute_sync_committee_duty(decision, state)
    ...
    audit_duty_event(:consensus_completed, extract_cluster_id_from_key(duty_key), %{
  ...
  defp initiate_view_change(consensus_state) do
    ...
    view_change_message = create_consensus_message(
      :view_change,
      consensus_state.duty_assignment,
      new_view,
      get_node_id()
  ...
  defp handle_consensus_failure(duty_key, reason, state) do
    cluster_id = extract_cluster_id_from_key(duty_key)
    
    audit_duty_event(:consensus_failed, cluster_id, %{
      duty_key: duty_key,
      reason: reason,
  ...
  defp audit_duty_event(event_type, cluster_id, metadata, audit_config) do
    case audit_config do
      %{} = config when map_size(config) > 0 ->
        AuditLogger.log_event(:dvt_consensus, event_type, %{
          cluster_id: cluster_id,
          timestamp: DateTime.utc_now(),
          metadata: metadata
        }, config)
      ...
      _ ->
        Logger.info("DVT Consensus Event: #{event_type} for cluster #{cluster_id}", metadata: metadata)
    end
  end
end