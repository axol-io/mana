defmodule ExWire.DVT.ByzantineTolerance do
  ...
  # Server state
  defstruct [
    :cluster_configs,        # %{cluster_id => bft_config}
    :node_states,           # %{cluster_id => %{node_id => node_state}}
    :fault_evidence,        # List of fault evidence records
    :view_states,           # %{cluster_id => current_view_info}
    :recovery_procedures,   # Active recovery procedures
    :performance_stats,     # BFT performance monitoring
  ...
  @type bft_config :: %{
    ...
    max_faulty_nodes: pos_integer(),        # f in BFT literature
    safety_threshold: pos_integer(),        # 2f + 1 for safety
    liveness_threshold: pos_integer(),      # f + 1 for liveness
    view_change_timeout: pos_integer(),     # Milliseconds before triggering view change
    leader_timeout: pos_integer(),          # Milliseconds before suspecting leader failure
    message_timeout: pos_integer(),         # Milliseconds for message delivery
    max_view_changes: pos_integer(),        # Maximum view changes before escalation
    recovery_strategy: :automatic | :manual | :hybrid        # :automatic | :manual | :hybrid
  }

  @type node_state :: %{
    node_id: String.t(),
    cluster_id: String.t(),
    status: :active | :suspected | :faulty | :recovered,                  # :active | :suspected | :faulty | :recovered
    last_seen: pos_integer(),
    message_count: pos_integer(),
    invalid_messages: pos_integer(),
    response_times: list(float()),          # Recent response time history
    view_changes_initiated: pos_integer(),
    fault_score: float(),            # Accumulated fault evidence
    recovery_attempts: pos_integer(),
    quarantine_until: pos_integer()        # DateTime when quarantine expires
  }

  @type fault_evidence :: %{
    node_id: String.t(),
    fault_type: atom(),
    evidence_data: map(),          # Specific evidence for this fault
    reported_by: String.t(),            # Node that reported the fault
    timestamp: pos_integer(),
    severity: :low | :medium | :high | :critical,               # :low | :medium | :high | :critical
    verified: boolean(),               # Whether evidence has been verified
    resolution: String.t()              # How the fault was resolved
  }
  ...
  def report_suspicious_behavior(cluster_id, suspected_node, fault_type, evidence) do
    GenServer.cast(__MODULE__, {:report_suspicious_behavior, cluster_id, suspected_node, fault_type, evidence})
  end
  ...
  def handle_call({:configure_bft, cluster_id, total_nodes, bft_options}, _from, state) do
    max_faulty = div(total_nodes - 1, 3) # Classical BFT: n >= 3f + 1
    ...
    node_states = Enum.reduce(0..(total_nodes - 1), %{}, fn node_id, acc ->
      node_state = %{
        node_id: node_id,
        cluster_id: cluster_id,
        status: :active,
    ...
    new_state = %{state |
      ...
      view_states: Map.put(state.view_states, cluster_id, %{
        current_view: 0,
        leader_id: 0,
    ...
    audit_bft_event(:bft_configured, cluster_id, %{
      total_nodes: total_nodes,
      max_faulty_nodes: max_faulty,
  ...
  def handle_call({:process_node_message, cluster_id, node_id, message}, _from, state) do
    case get_cluster_node_state(cluster_id, node_id, state) do
      {:ok, node_state} ->
        ...
        response_time = DateTime.diff(current_time, Map.get(message, :sent_at, current_time), :millisecond)
        
        updated_node = %{node_state |
        ...
        case validate_message_integrity(message, cluster_id, node_id, state) do
          ...
          {:invalid, reason} ->
            ...
            report_fault_evidence(cluster_id, node_id, :invalid_message, %{
              reason: reason,
              message: message
            }, new_state)
  ...
  def handle_call({:trigger_recovery, cluster_id, recovery_type}, _from, state) do
    case Map.get(state.cluster_configs, cluster_id) do
      ...
      bft_config ->
        ...
        new_state = %{state |
          recovery_procedures: Map.put(state.recovery_procedures, recovery_ref, recovery_procedure)
        }
        ...
        audit_bft_event(:recovery_triggered, cluster_id, %{
          recovery_type: recovery_type,
          recovery_ref: recovery_ref
        }, state.audit_config)
  ...
  @impl true
  def handle_cast({:report_suspicious_behavior, cluster_id, suspected_node, fault_type, evidence}, state) do
    fault_evidence = %{
    ...
    updated_state = case get_cluster_node_state(cluster_id, suspected_node, new_state) do
      {:ok, node_state} ->
    ...
    final_state = check_and_trigger_fault_response(cluster_id, suspected_node, fault_type, updated_state)

    audit_bft_event(:suspicious_behavior_reported, cluster_id, %{
  ...
  def handle_info(:fault_detection, state) do
    ...
    new_state = Enum.reduce(state.cluster_configs, state, fn {cluster_id, _config}, acc_state ->
  ...
  def handle_info({:recovery_completed, recovery_ref, result}, state) do
    case Map.get(state.recovery_procedures, recovery_ref) do
      ...
      recovery_procedure ->
        completed_procedure = %{recovery_procedure |
          status: :completed,
        ...
        new_state = %{state |
          recovery_procedures: Map.put(state.recovery_procedures, recovery_ref, completed_procedure)
        }

        audit_bft_event(:recovery_completed, recovery_procedure.cluster_id, %{
          recovery_ref: recovery_ref,
          result: result,
          duration_ms: DateTime.diff(DateTime.utc_now(), recovery_procedure.started_at, :millisecond)
        }, state.audit_config)
  ...
  defp calculate_fault_score(node_state, fault_type) do
    ...
    fault_increment = case fault_type do
      :invalid_message -> 0.1
      :slow_response -> 0.05
      :byzantine -> 0.5
  ...
  defp report_fault_evidence(cluster_id, node_id, fault_type, evidence, state) do
    GenServer.cast(self(), {:report_suspicious_behavior, cluster_id, node_id, fault_type, evidence})
  end
  ...
  defp calculate_quarantine_time(fault_type, fault_score) do
    base_minutes = case fault_type do
      :invalid_message -> 5
      :slow_response -> 2
      :byzantine -> 30
  ...
  defp check_and_trigger_fault_response(cluster_id, suspected_node, fault_type, state) do
    case Map.get(state.cluster_configs, cluster_id) do
      nil ->
        state
      ...
      bft_config ->
        node_states = Map.get(state.node_states, cluster_id, %{})
        faulty_count = Enum.count(node_states, fn {_id, node} -> 
  ...
  defp initiate_view_change(cluster_id, current_view_or_atom, reason, state) do
    case Map.get(state.view_states, cluster_id) do
      ...
      view_state ->
        current_view = if current_view_or_atom == :current do
        ...
        audit_bft_event(:view_change_initiated, cluster_id, %{
  ...
  defp trigger_emergency_recovery(cluster_id, state) do
    ...
    audit_bft_event(:emergency_recovery_triggered, cluster_id, %{
  ...
  defp perform_fault_detection(cluster_id, state) do
    case Map.get(state.node_states, cluster_id) do
      ...
      node_states ->
        ...
        updated_nodes = Enum.reduce(node_states, %{}, fn {node_id, node_state}, acc ->
  ...
  defp detect_node_faults(node_state, current_time) do
    ...
    if time_since_seen > 60 and node_state.status == :active do
      report_fault_evidence(node_state.cluster_id, node_state.node_id, :crash, %{
  ...
  defp calculate_cluster_avg_response_time(node_states) do
    all_response_times = Enum.flat_map(node_states, fn {_id, node} -> 
      node.response_times 
    end)
  ...
  defp calculate_cluster_message_throughput(node_states) do
    total_messages = Enum.sum(Enum.map(node_states, fn {_id, node} -> 
      node.message_count 
    end))
  ...
  defp calculate_fault_detection_accuracy(cluster_id, state) do
  ...
  defp get_recent_fault_count(cluster_id, state) do
    cutoff_time = DateTime.add(DateTime.utc_now(), -300, :second) # Last 5 minutes
  ...
  defp execute_recovery_procedure(recovery_procedure, bft_config, server_pid) do
    ...
    try do
      ...
      result = Enum.reduce_while(steps, {:ok, []}, fn step, {:ok, completed_steps} ->
  ...
  defp audit_bft_event(event_type, cluster_id, metadata, audit_config) do
    case audit_config do
      %{} = config when map_size(config) > 0 ->
        AuditLogger.log_event(:dvt_byzantine_tolerance, event_type, %{
          cluster_id: cluster_id,
          timestamp: DateTime.utc_now(),
          metadata: metadata
        }, config)
