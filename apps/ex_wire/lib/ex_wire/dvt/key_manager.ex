defmodule ExWire.DVT.KeyManager do
  ...
  defstruct [
    :clusters,          # %{cluster_id => cluster_config}
    :key_shares,        # %{cluster_id => %{node_id => key_share_data}}
    :hsm_config,        # HSM configuration
    :audit_config,      # Audit logging configuration
    :rbac_config,       # RBAC configuration
    :rotation_schedule, # Key rotation schedule
  ...
  """
  @spec create_cluster(cluster_id(), validator_pubkey(), pos_integer(), pos_integer(), 
                      list(map()), keyword()) :: {:ok, map()} | {:error, atom()}
  def create_cluster(cluster_id, validator_pubkey, threshold, total_nodes, participants, opts \\ []) do
    GenServer.call(__MODULE__, {:create_cluster, {
  ...
  @impl true
  def handle_call({:create_cluster, {cluster_id, validator_pubkey, threshold, total_nodes, participants, opts}}, 
                  _from, state) do
    case validate_cluster_creation(cluster_id, threshold, total_nodes, participants, state) do
      :ok ->
        case create_cluster_impl(cluster_id, validator_pubkey, threshold, total_nodes, participants, opts, state) do
          {:ok, cluster_config, new_state} ->
            # Audit log cluster creation
            audit_cluster_event(:cluster_created, cluster_id, %{
          ...
          {:error, reason} = error ->
            audit_cluster_event(:cluster_creation_failed, cluster_id, %{reason: reason}, state.audit_config)
            {:reply, error, state}
        end
      ...
      cluster_config ->
        case initialize_dkg_impl(cluster_config, opts, state) do
          {:ok, dkg_data, new_state} ->
            audit_cluster_event(:dkg_initialized, cluster_id, %{
          ...
          {:error, reason} = error ->
            audit_cluster_event(:dkg_initialization_failed, cluster_id, %{reason: reason}, state.audit_config)
            {:reply, error, state}
        end
    end
  end
  ...
  def handle_call({:sign_message, cluster_id, message, opts}, _from, state) do
    with {:ok, cluster_config} <- get_cluster_config(cluster_id, state),
      ...
      audit_cluster_event(:message_signed, cluster_id, %{
      ...
      {:error, reason} = error ->
        audit_cluster_event(:signing_failed, cluster_id, %{
          reason: reason,
          message_hash: :crypto.hash(:sha256, message) |> Base.encode16()
        }, state.audit_config)
        {:reply, error, state}
  ...
  def handle_call({:rotate_keys, cluster_id, opts}, _from, state) do
    with {:ok, cluster_config} <- get_cluster_config(cluster_id, state),
      ...
      audit_cluster_event(:keys_rotated, cluster_id, %{
      ...
      {:error, reason} = error ->
        audit_cluster_event(:key_rotation_failed, cluster_id, %{reason: reason}, state.audit_config)
        {:reply, error, state}
    end
  end
  ...
  defp create_cluster_impl(cluster_id, validator_pubkey, threshold, total_nodes, participants, opts, state) do
      ...
      {:ok, key_shares} when length(key_shares) >= threshold ->
        ...
        signature_shares = 
          ...
          |> Enum.map(fn {_node_id, key_share} ->
            case Crypto.create_dvt_signature_share(key_share.share_data, message, state.hsm_config) do
  ...
  defp perform_key_rotation(cluster_config, opts, state) do
    ...
    with {:ok, dkg_data, intermediate_state} <- initialize_dkg_impl(cluster_config, opts, state),
      ...
      new_cluster_config = %{cluster_config | 
  ...
  defp calculate_next_rotation(compliance_level) do
    rotation_interval = case compliance_level do
      :standard -> 90 # 90 days
      :enterprise -> 30 # 30 days  
      :regulated -> 14 # 14 days
    end
  ...
  defp audit_cluster_event(event_type, cluster_id, metadata, audit_config) do
    case audit_config do
      %{} = config when map_size(config) > 0 ->
        AuditLogger.log_event(:dvt_operation, event_type, %{
          cluster_id: cluster_id,
  ...
  defp schedule_key_rotation_checks do
    Process.send_after(self(), :rotation_check, 3_600_000) # Every hour
  end
