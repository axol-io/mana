defmodule ExWire.DVT.P2PProtocol do
  ...
  @type message_type :: :key_gen | :duty_consensus | :slashing_alert | :heartbeat | :performance_metrics
  ...
  defstruct [
    :node_id,
    :cluster_memberships,    # %{cluster_id => %{role, permissions}}
    :peer_connections,       # %{peer_id => connection_info}
    :message_cache,          # Recently seen message IDs for replay protection
    :authentication_keys,    # Node authentication keys
    :gossipsub_pid,         # GossipSub process PID
    :monitoring_metrics,    # Performance and network metrics
  ...
  def send_direct_message(cluster_id, target_node_id, message_type, payload) do
    GenServer.call(__MODULE__, {:direct_message, cluster_id, target_node_id, message_type, payload})
  end
  ...
  @impl true
  def handle_call({:join_cluster, cluster_id, node_id, auth_key}, _from, state) do
  ...
  @impl true
  def handle_call({:leave_cluster, cluster_id}, _from, state) do
    case Map.get(state.cluster_memberships, cluster_id) do
      ...
      _membership ->
        ...
        {:reply, :ok, %{state | 
          ...
          peer_connections: new_connections
        }}
    end
  end

  @impl true
  def handle_call({:broadcast, cluster_id, message_type, payload}, _from, state) do
    case Map.get(state.cluster_memberships, cluster_id) do
      nil ->
        {:reply, {:error, :not_member}, state}
          ...
          true ->
            ...
            case GossipSub.publish(state.gossipsub_pid, topic, :erlang.term_to_binary(message)) do
              :ok ->
                ...
                {:reply, :ok, %{state | monitoring_metrics: metrics}}
  ...
  @impl true
  def handle_call({:direct_message, cluster_id, target_node_id, message_type, payload}, _from, state) do
  ...
  @impl true
  def handle_info({:gossipsub, topic, message_data}, state) do
  ...
  defp create_authenticated_message(cluster_id, message_type, payload, state) do
  ...
  defp has_permission?(membership, message_type) do
    required_permission = case message_type do
  ...
  defp announce_presence(cluster_id, node_id, state) do
    presence_message = %{
      node_id: node_id,
      public_key: state.authentication_keys.public,
  ...
  defp handle_dvt_message(topic, message, state) do
    ...
    case verify_message_signature(message, state) do
      {:ok, verified_message} ->
        ...
        case check_replay_protection(verified_message, state) do
          :ok ->
            process_verified_message(topic, verified_message, state)
  ...
  defp check_replay_protection(message, state) do
    ...
    case :ets.lookup(state.message_cache, message_id) do
      [] ->
        ...
        :ets.insert(state.message_cache, {message_id, timestamp})
        :ok
  ...
  defp find_peer_by_node_id(cluster_id, node_id, state) do
    result = 
      state.peer_connections
  ...
  defp cleanup_stale_connections(state) do
    cutoff = DateTime.add(DateTime.utc_now(), -60, :second) # 1 minute
  ...
  defp handle_performance_metrics(_message, state) do
