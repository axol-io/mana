defmodule ExWire.DVT.PartitionDetector do
  ...
  @consensus_timeout_ms 30_000      # 30 seconds
  @partition_threshold 0.33         # 33% of nodes unreachable = partition
  @recovery_probe_interval_ms 5_000 # 5 seconds
  ...
  defstruct [
    :node_id,
    :cluster_memberships,     # %{cluster_id => cluster_info}
    :partition_states,        # %{cluster_id => partition_state}
    :heartbeat_tracking,      # %{cluster_id => %{node_id => last_heartbeat}}
    :consensus_tracking,      # %{cluster_id => consensus_state}
    :recovery_attempts,       # %{cluster_id => attempt_count}
    :partition_history,       # List of recent partition events
    :topology_cache,          # Network topology information
  ...
  def monitor_cluster(cluster_id, node_count, threshold, partition_tolerance \\ :minority) do
    GenServer.call(__MODULE__, {:monitor_cluster, cluster_id, node_count, threshold, partition_tolerance})
  end
  ...
  @impl true
  def handle_call({:unmonitor_cluster, cluster_id}, _from, state) do
    new_state = %{state |
  ...
  @impl true
  def handle_call({:trigger_recovery, cluster_id}, _from, state) do
  ...
  @impl true
  def handle_cast({:heartbeat, cluster_id, node_id}, state) do
  ...
  @impl true
  def handle_cast({:consensus_activity, cluster_id, round, participating_nodes}, state) do
    case Map.get(state.consensus_tracking, cluster_id) do
      ...
      consensus_state ->
        ...
        missing_nodes = MapSet.difference(cluster_info.expected_nodes, MapSet.new(participating_nodes))
  ...
  @impl true
  def handle_info(:probe_recovery, state) do
  ...
  defp parse_monitoring_config(opts) do
    %{
      ...
      recovery_probe_interval: Keyword.get(opts, :recovery_probe_interval, @recovery_probe_interval_ms),
      max_recovery_attempts: Keyword.get(opts, :max_recovery_attempts, @max_recovery_attempts)
    }
  end

  defp check_all_clusters_for_partitions(state) do
  ...
  defp check_cluster_partition(cluster_id, state) do
    cluster_info = Map.get(state.cluster_memberships, cluster_id)
    ...
    {reachable_nodes, unreachable_nodes} = analyze_heartbeat_connectivity(cluster_info, heartbeats)
    ...
    if new_partition_state != current_state do
      ...
      %{state | partition_states: Map.put(state.partition_states, cluster_id, new_partition_state)}
    else
      state
    end
  end
  ...
  defp determine_partition_state(cluster_info, reachable_nodes, unreachable_nodes, consensus_partition) do
  ...
  defp handle_partition_state_change(cluster_id, old_state, new_state, state) do
    Logger.warning("DVT cluster partition state changed", %{
      cluster_id: cluster_id,
      old_state: old_state,
      new_state: new_state
    })
  ...
  defp maybe_trigger_auto_recovery(cluster_id, state) do
    cluster_info = Map.get(state.cluster_memberships, cluster_id)
  ...
  defp trigger_partition_recovery(cluster_id, state) do
    attempts = Map.get(state.recovery_attempts, cluster_id, 0)
    
    if attempts < @max_recovery_attempts do
      Logger.info("Triggering partition recovery for cluster #{cluster_id} (attempt #{attempts + 1})")
  ...
  defp verify_recovery_success(cluster_id) do
    ...
    case get_cluster_partition_status(cluster_id) do
      ...
      {:ok, status} ->
        Logger.warning("Partition recovery incomplete for cluster #{cluster_id}: #{inspect(status)}")
  ...
  defp maybe_resolve_partition(cluster_id, state) do
  ...
  defp probe_partition_recovery(state) do
    recovering_clusters = 
  ...
  defp check_recovery_progress(cluster_id, state) do
  ...
  defp get_reachable_nodes(cluster_id, state) do
    cluster_info = Map.get(state.cluster_memberships, cluster_id)
    heartbeats = Map.get(state.heartbeat_tracking, cluster_id, %{})
  ...
  defp get_unreachable_nodes(cluster_id, state) do
    cluster_info = Map.get(state.cluster_memberships, cluster_id)
    heartbeats = Map.get(state.heartbeat_tracking, cluster_id, %{})
  ...
  defp count_partitioned_clusters(state) do
    state.partition_states
    |> Enum.count(fn {_cluster_id, partition_state} -> partition_state == :partitioned end)
  end

  defp get_last_partition_event(state) do
