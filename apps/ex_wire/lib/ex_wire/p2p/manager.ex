  ...
  defp get_packet(session, message_id, packet_data) do
    case PacketIdMap.get_packet_module(session.packet_id_map, message_id) do
      {:ok, packet_mod} ->
        ...
        packet =
          if should_handle_request_id?(packet_mod, session.negotiated_version) do
    ...
    packet_data =
      if should_handle_request_id?(packet_mod, session.negotiated_version) do
        ...
        {wrapped_packet, request_id} =
          ExWire.Packet.Capability.Eth.V66Wrapper.wrap_packet(
            packet,
            session.negotiated_version
          )
        ...
        ExWire.Packet.Capability.Eth.V66Wrapper.serialize(
          wrapped_packet,
          session.negotiated_version
        )
      else
