defmodule VerkleTree.NativeCore do
  ...
  def initialize(node_buffer_size \\ 4096, pool_size \\ 1000) do
    Logger.info("Initializing VerkleTree.NativeCore with buffer_size=#{node_buffer_size}, pool_size=#{pool_size}")
  ...
  def benchmark(operation_count \\ 10_000) do
    ...
    # Insert benchmark
    {insert_time, {:ok, _insert_results}} = :timer.tc(fn -> 
    ...
    # Read benchmark  
    {read_time, {:ok, _read_results}} = :timer.tc(fn ->
      batch_read(keys)
    end)
    ...
    {witness_time, {:ok, _witnesses}} = :timer.tc(fn ->
